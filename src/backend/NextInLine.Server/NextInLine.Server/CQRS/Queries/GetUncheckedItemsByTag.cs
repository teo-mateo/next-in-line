...
public class GetUncheckedItemsByTagQueryHandler : IRequestHandler<GetUncheckedItemsByTagQuery, IEnumerable<Item>>
{
    ...
    public async Task<IEnumerable<Item>> Handle(GetUncheckedItemsByTagQuery request, CancellationToken cancellationToken)
    {
        // SQL query: Retrieves all unchecked items that have the specified tag
        // The query joins the Items, ItemTags, and Tags tables
        const string sql = @"SELECT i.Id, i.Name, i.WhenAdded, i.AddedBy, i.Checked, i.WhenChecked, t.Id AS TagId, t.TagName
                             FROM Items i
        ...
        var items = await _connection.QueryAsync<Item, Tag, Item>(sql,
            (item, tag) =>
            {
